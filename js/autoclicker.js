class DiplomaAutoclicker {
    ...
    addText() {
        ...
        // Проверяем, нужно ли слово с большой буквы
        let needCapital = false;
        ...
        // После точки
        else if (this.currentText.trimEnd().endsWith('.')) {
            needCapital = true;
        }
        // После нового абзаца (\n\n)
        ...
        // После красной строки (пробелы в начале абзаца)
        else if (this.currentText.endsWith('    ')) {
            needCapital = true;
        }
        
        // Делаем большую букву если нужно
        if (needCapital) {
        ...
        if (!needCapital && Math.random() < 0.05 && this.currentText.length > 100) {
            ...
            this.currentText += '\n\n    '; // Новый абзац с красной строки
            isNewParagraph = true;
            // Делаем слово с большой буквы для нового абзаца
            word = word.charAt(0).toUpperCase() + word.slice(1);
            document.getElementById('currentAction').textContent = 'Начинаю новый абзац...';
        }
        
        // Если это самый первый абзац — начинаем с красной строки
        if (this.currentText.length === 0) {
            this.currentText = '    '; // Красная строка
        }
        
        // Добавляем слово
        if (isNewParagraph) {
            // Красная строка уже добавлена, просто слово
            this.currentText += word + punctuation + ' ';
        } else if (punctuation === '.') {
            // После точки добавляем два пробела (для читаемости) и следующее слово будет с большой буквы
            this.currentText += word + punctuation + '  ';
        } else {
            this.currentText += word + punctuation + ' ';
        }
